...
| Partners (programme) | /admin/partners, /admin/partners/{userId} (+remind), /admin/partners/claims/{id}/confirm\|reject, /admin/partners/remind-behind | ✅ | ✅ | **2026-09-23** (pathway #482): every partner, their pledges + due commitments + statements, send reminder (12 h spacing), remind everyone behind, "I paid another way" claims queue. iPad = same bundle (Capacitor). |
| Departments | /admin/departments (+{id}), serve-requests, needs (+{needId}/approve\|reject\|close), office posts | ✅ | ✅ | **2026-09-23** (pathway #483): CRUD, leader, gift keys, requests-to-serve queue, financial-needs queue (approved need = first-class giving target via `need_id`). Office bounded by congregation (§5.4). iPad = same bundle. |
