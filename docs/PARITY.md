...
| **Finance ERP** (sidebar group FINANCE, 17 pages) | /admin/finance/* (overview, transactions + gifts + reverse, pledges, funds + transfers + opening-balances, journals, ledger, trial-balance, reconciliation, reports/income\|expenses\|pledges\|financial-position\|income-expenditure, statements + PDFs, needs, givers, expenses, expense-categories, budgets, audit, settings), /admin/permissions/catalog, /admin/campaigns*, /admin/partners* | ✅ | ✅ | **2026-09-26** (migration 216, docs/FINANCE_ERP.md): Finance leaves Operations and becomes its own group; the same day it was regrouped (owner) into three folding sub-menus — **Giving & Income** (Overview · Transactions · Pledges · Partners · Claims · Recurring gifts · Campaigns), **Spending & Planning** (Department needs · Expenses · Budgets · Funds), **Accounting & Reporting** (Ledger · Reconciliation · Reports · Statements · Audit) — with **Settings** on its own at the bottom. Folded by default, the current page's sub-menu opens itself, folds remembered per device (§1). Double-entry journals (expense, void, transfer, opening, reversal), office gifts with gapless OR- receipts, reversal-only corrections, maker-checker expenses, budgets vs actuals, financial statements, reconciliation exceptions. finance:view / export / manage / approve. Role editors (web Roles + Users, iPad Roles) render from the permissions catalog so saving never strips `manage`/`go`. Web: pathway feat/finance-erp; iPad: pathwayforipad feat/finance-erp (native pages, same content). |
...
**Tab bar since 2026-09-23 (both apps):** Home · Pathway · Plans · Events · Give · You. Live moved off the bar (broadcast card on Home/Events); **You** = Community · Departments · Profile · Settings; **Give** = Give · Partners. Deep links `openEvents/openGive/openPartners/openDepartment` on both.
...
| Give | Giving, GivingStatement, GivingReceipt, **Partners** (iOS `PartnersView` / Android `PartnersScreen`), **PartnerInviteSheet** (both, hosted on Home) | /giving/* — **Partners phase 1 (2026-09-02):** `GET /giving/partnership` derives standing from `giving_schedules` (never stored twice); `kept` counts collected cycles only; `since_you_began` is church-wide, never attributed. **Phase 2 (2026-09-03):** `campaigns` + `partner_invite_log` (migration 212); `GET /giving/invitation` decides *whether* to ask (all restraint rules server-side, 24 tests); `/giving/invitation/{id}/shown|outcome`; admin `/admin/campaigns*` (draft-until-live, `reach`). Tiers from `tiers.ts` off one costing (KSh 20,000/disciple/level), rounded DOWN. Both clients render only what the server returns. **Partners programme (2026-09-23, pathway #482):** join with or without a target; several pledges per member (`pledges`, monthly\|total, each with its own ledger via `transactions.pledge_id`); auto-schedule bound to a pledge; reminders 3 days before + up to 3 polite follow-ups 12 h apart (`pledge_reminders`); "I paid another way" claims; statements by year. Android recurring-gift bug fixed in the same change (was posting a one-time intent). Screens: `PartnersView`/`PartnersScreen` rebuilt, `NewPledgeFlow` both. **UI v2 (2026-09-24):** one header band, GIVE|PARTNERS control, eye on the year pill, Partners v2 (spec §3a). **Pledge names + server-routed pledge money (2026-09-25, migration 215, spec §3b):** `pledge_options` picker + custom name, `custom_title`/`title` on pledges, intent result `fund`/`pledge`; both apps render the ceremony from the result, never the fund chip. **Receipt v2 (2026-09-25):** `GET /giving/transactions/{id}` gains `fund_name`, `method_label`, `pledge`, `need`, `member_name`, `congregation`; members never see ledger account codes; share = `/giving/transactions/{id}/receipt.pdf`. **Separate statements (2026-09-25, spec §3c):** Partners statement screen + `GET /giving/partners/statement.pdf?year=`; `/giving/statements` gains server-side pledged/paid/remaining + `pledges[]`; history rows gain `pledge_id`/`pledge_title`; Partners links open the Partners statement, the giving statement stays the complete record. **Statement v2 (2026-09-25, spec §3d):** impact-led Partners statement (`impact`, `months[12]`, `faithfulness`, `season`, per-pledge remaining/church progress; two-page PDF); Giving statement splits Gifts · Partner pledges · Total with a collapsed pledge group (PDF section). |
| **Departments** (You tab) | iOS `DepartmentsView` + `DepartmentDetailView` / Android `DepartmentsScreen` + `DepartmentScreen`; Profile "Serving in" card | /departments, /departments/{id} (+serve, posts, needs, serve-requests decisions), /me/departments — **2026-09-23** (pathway #483): gift fit from the latest gifts assessment, leader posts + needs, "Give to this need" → `need_id` on the intent (fund decided by the server). |
