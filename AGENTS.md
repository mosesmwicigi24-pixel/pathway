...
the **native iPad app** (separate repo at `/Users/mwicigi/Codex/Projects/iphone`), the mobile
...
This project builds the Nuru Place Pathway Discipleship platform. The attached nuru-place-technical-spec.pdf is the source of truth for architecture, schema, API, infrastructure, and security — follow it and flag any deviation. Stack: React Native (iOS/Android) + React (admin web) + Node/TypeScript backend + PostgreSQL/Redis, all TypeScript in one monorepo. Prefer production-quality, build-ready code with tests. Keep the offline-first sync engine, server-authoritative gating, and the PCI-SAQ-A money path intact as specified. Ask before adding dependencies or changing the spec's decisions.
