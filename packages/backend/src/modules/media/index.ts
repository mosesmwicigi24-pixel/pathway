...
export function registerMedia(ctx: AppContext): Router {
  ...
  // url/duration on the radio_program. Cap 250 MB; formats limited to MP3, WAV,
  // AAC, and ALAC/AAC in an .m4a container (product decision 2026-07-03).
  const AUDIO_MAX_BYTES = 250 * 1024 * 1024; // full sessions/sermons; nginx location caps at 260m
    ...
    audioUpload.single("file")(req, res, (err: unknown) => {
      ...
      if (err instanceof multer.MulterError) {
        return next(
          new ApiError(
            "VALIDATION_FAILED",
            err.code === "LIMIT_FILE_SIZE" ? "Audio exceeds the 250 MB upload limit" : err.message,
          ),
        );
      }
      return next(err);
